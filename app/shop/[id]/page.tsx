...
export default function Shop({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    supabase.from("sellers").select("*").eq("id", params.id).single().then(({ data }) => { if (data) setSeller(data); });
    supabase.from("products").select("*").eq("seller_id", params.id).eq("active", true).then(({ data }) => { if (data) setProducts(data); });
    supabase.from("ratings").select("rating").eq("seller_id", params.id).then(({ data }) => {
      if (data && data.length) {
        setAvg(data.reduce((s, r) => s + r.rating, 0) / data.length);
        setRcount(data.length);
      }
    });
  }, [params.id]);
  return (
      ...
      <main className="flex-1 p-4 space-y-3">
        ...
        {seller && (
          <div className="gold-card rounded-2xl p-4">
            ...
            {rcount > 0 && <p className="text-sm font-bold text-amber-600 mt-1">⭐ {avg.toFixed(1)} ({rcount} ratings)</p>}
          </div>
        )}
        <div className="space-y-3">
          {products.map((p) => (
            <div key={p.id} className="border-2 border-gray-200 rounded-2xl p-4 flex items-center gap-3">
              ...
              <Link href={"/order/" + p.id} className="gold-btn text-white text-sm font-bold px-4 py-2.5 rounded-full">Order Karo</Link>
            </div>
          ))}
        </div>
      </main>
