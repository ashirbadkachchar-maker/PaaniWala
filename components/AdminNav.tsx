...
export default function AdminNav() {
  ...
  const logout = () => { localStorage.removeItem("pw_admin"); router.push("/admin"); };
  return (
    <div className="bg-blue-900 text-white sticky top-0 z-10">
      ...
      <nav className="grid grid-cols-6 text-center text-xs">
        {items.map((it) => (
          <Link key={it.href} href={it.href} className={"py-2 " + (path === it.href? "bg-amber-500 text-blue-900 font-bold" : "text-amber-100")}>
